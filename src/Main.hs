...
escapedChars = do char '\\'  -- a backslash
                  x <- oneOf "\\\"nrt" -- backslash or doublequote
                  return $ case x of
                    '\\' -> x
                    '"' -> x
                    'n' -> '\n'
...
showVal (DottedList head tail) = "(" ++ unwordsList head ++ " . " ++
    showVal tail ++ ")"
showVal (PrimitiveFunc _) = "<primitives>"
showVal (Func {params = args, vararg = varargs, body = body, closure = 
  env}) = 
...
symbolp, numberp, boolp, listp :: LispVal -> LispVal
...
evalString env expr = runIOThrows $ liftM show $ (liftThrows $
...
primitiveBindings = nullEnv >>= (flip bindVars $ map (makeFunc IOFunc) ioPrimitives ++  map (makeFunc PrimitiveFunc)
  primitives)
...
runIOThrows action = runErrorT (trapError action) >>= return .
  extractValue
...
getVar envRef var = do env <- liftIO $ readIORef envRef
                       maybe (throwError $ UnboundVar "Getting an unbounded variable" var)
                            (liftIO . readIORef)
                            (lookup var env)
...
makeFunc varargs env params body = return $ Func (map showVal params)
  varargs body env
